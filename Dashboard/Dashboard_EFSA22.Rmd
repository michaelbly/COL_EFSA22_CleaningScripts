---
title: 'WFP-CNC EFSA22: Recogida de Datos - 04/07/2022 10:00'
output:
...
dat <- read.csv("Input/efsa_all_data_2022-07-04.csv",
...
interview_method_pendulario <- dat %>%
  dplyr::filter(pop_group == "pendulario") %>% 
  dplyr::select(Fecha = date_assessment,
                Entrevistador = entrevistador,
                Departamento = departamento,
                population_group = pop_group
...
interview_method_date_pendulario <- dat %>% 
  dplyr::filter(date_assessment >= assessment_start_date & pop_group == "pendulario") %>% 
  dplyr::select(Fecha = date_assessment,
                Entrevistador = entrevistador,
                Departamento = departamento,
                population_group = pop_group) %>% 
...
<center>**Mapa porcentaje de encuestas completadas (todos los grupos de población combinados) por departamento** </center> </br>
...
leaflet(options = leafletOptions(zoomControl = FALSE,
  ...
  addPolygons(data=df.map, color = ~pal.strata(df.map$pct.done),
              ...
              label=paste0(df.map$departamento, ": ", round(df.map$pct.done,  1), "% completado (",
                           df.map$Restante, " restante)")) %>%
  addTiles() %>%
